...
function isStringFilled(value) {
  return isString$2(value) && value.trim() !== "";
}
...
function isObject$1(value) {
  return typeof value === "object" && !isNull(value) && !isArray$5(value);
}
...
function isStringContainsString(haystack, needle) {
  if (!isString$2(haystack) || !isString$2(needle)) {
    return false;
  }
...
function isNumberZero(value) {
  return isNumber$2(value) && value == 0;
}
...
var inspectCustom = _nodeResolve_empty$1.custom;
...
function urlSafeBase64Encode(value) {
  return encode$2(value).replaceAll('+', '-').replaceAll('/', '_').replaceAll('=', '.');
}
function urlSafeBase64Decode(value) {
  return decode$1(value.replaceAll('.', '=').replaceAll('_', '/').replaceAll('-', '+'));
}
...
function urlGetSearchParams(wls) {
  return lib.parse(urlGetSearchString(wls));
}
...
function swalToast(config) {
  ...
  return sweetalert2_all.fire(mergeConfig);
}
...
function ajaxRequest(method, url, config) {
...
function ajaxGet(url, query, config) {
  if (isNil(query)) {
    query = {};
  }
...
function ajaxPost(url, data, config) {
  if (isNil(config)) {
    config = {};
  }
  ...
  const mergeConfig = _extends({
    data: data
  }, config); // if (!isObject(mergeConfig.headers)) {
  //   mergeConfig.headers = {};
  // }
...
function ajaxPut(url, data, config) {
  if (isNil(config)) {
    config = {};
  }
...
function ajaxPatch(url, data, config) {
  if (isNil(config)) {
    config = {};
  }
...
function ajaxHandleError(resp) {
  ...
  if (resp instanceof Error) {
    ...
    if (isObject$1(resp.response) && !isNil(resp.response.data)) {
      resp = resp.response;
    }
  }
  ...
  const respData = _extends({}, resp).data;

  if (!isObject$1(respData)) {
    swalAlert({
  ...
  if (respData.retCode >= 500) {
    ...
    if (Array.isArray(respData.data)) {
    ...
    } else if (isObject$1(respData.data)) {
      ...
      } else if (isObject$1(respData.data.err)) {
        const errors = Object.values(respData.data.err);
...
const _onFulfilled = resp => {
  if (resp.config.responseType === 'blob') {
    return resp;
  } // normalize headers


  const respHeaders = _extends({}, resp.headers);

  const stdHeaders = {};
  Object.keys(respHeaders).forEach(k => {
    const name = k.toLowerCase();
    let value = respHeaders[k]; // for special headers
...
export { ajaxGet, ajaxHandleError, ajaxPatch, ajaxPost, ajaxPut, ajaxRequest, isArray$5 as isArray, isArrayEmpty, isArrayFilled, isBoolean$1 as isBoolean, isFalse, isFalsyValue, isFunction$1 as isFunction, isInteger, isIntegeric, isNil, isNull, isNumber$2 as isNumber, isNumberZero, isObject$1 as isObject, isObjectEmpty, isObjectFilled, isPlainObject$1 as isPlainObject, isString$2 as isString, isStringContainsChChars, isStringContainsString, isStringEmpty, isStringFilled, isSymbol$1 as isSymbol, isTrue, isUndefined$1 as isUndefined, swalAlert, swalToast, urlGetSearchParam, urlGetSearchParams, urlGetSearchString, urlSafeBase64Decode, urlSafeBase64Encode };
